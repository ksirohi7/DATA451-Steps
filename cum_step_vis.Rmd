...
```{r}
steps <- read.csv("new_full_predictions.csv")
...
get_sanity_plots <- function(data, ID, session) {
  ...
  test[,"pred_broad_steps"] <- cumsum(test$broad_steps)
  test[,"pred_granular_steps"] <- cumsum(test$gran_steps)
  test[,"pred_just_right_steps"] <- cumsum(test$just_right_steps)
  ...
  geom_line(aes(x=date_time, y=pred_cumul_steps, color="UK Biobank Predicted"), size=1) +
    geom_line(aes(x=date_time, y=pred_broad_steps, color="Keadle Trained Broad"), size=1) + 
    geom_line(aes(x=date_time, y=pred_just_right_steps, color="Keadle Trained Less Categories"), size=1) +
    geom_line(aes(x=date_time, y=pred_granular_steps, color="Keadle Trained Granular"), size=1) +
  labs(x="", y="",
  ...
  scale_x_datetime(date_breaks=db,
                   date_labels = "%H:%M:%S") +
    #"Keadle Trained Walk/Not Walk" = "green", "Keadle Trained All Postures" = "blue")
  scale_color_manual(name="", 
                     values = c("Actual" = "#a74d4eff", 
                                "UK Biobank Predicted" = "#1e619dff",
                                "Keadle Trained Broad" = "#c08080ff", 
                                "Keadle Trained Granular" = "#799943",
                                "Keadle Trained Less Categories" = "black"))+
  ...
  theme(#axis.text.y=element_blank(),
        ...
        text=element_text(family="arial"),
        plot.title = element_text(hjust = 0.5)) +
    scale_color_manual(values=c("walk with load" = "#799943",
                               "sitting" = "grey",
                               "lying" = "grey",
                               "kneeling/ squatting" = "grey",
                               "stretching" = "grey",
                               "walk" = "grey",
                               "ascend stairs" = "dodgerblue3",
                               "descend stairs" = "dodgerblue3",
                               "running" = "dodgerblue3",
                               "bike" = "grey",
